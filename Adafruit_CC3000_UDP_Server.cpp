...
Adafruit_CC3000_UDP_Server::Adafruit_CC3000_UDP_Server(uint8_t port) : _port(port) {
  bufsiz = 0;
  _rx_buf_idx = 0;
};
...
int16_t Adafruit_CC3000_UDP_Server::write(const void *buf, uint16_t len, uint32_t flags = 0) {
...
int16_t Adafruit_CC3000_UDP_Server::write(const void *buf, uint16_t len, sockaddr address) {
  ...
  int16_t ret = sendto(_socket, buf, len, 0, (sockaddr*)&address, slen);
  return ret;
};

int16_t Adafruit_CC3000_UDP_Server::read(void *buf, uint16_t len, uint32_t flags = 0) {
  return recv(_socket, buf, len, flags);
};

int16_t Adafruit_CC3000_UDP_Server::read(void *buf, uint16_t len, sockaddr address) {
  socklen_t slen = sizeof(address);
  int16_t ret = recvfrom(_socket, buf, len, 0, (sockaddr *)&address, &slen);
  return ret;
};

int16_t Adafruit_CC3000_UDP_Server::read(void) {
...
void Adafruit_CC3000_UDP_Server::begin(void) {
  ...
  cc3k_int_poll();
  if (netapp_timeout(&aucDHCP, &aucARP, &aucKeepalive, &aucInactivity) != 0 ) {
